...
typedef void  (*sched_setscheduler_nocheck_type) (struct task_struct *, int,
				      const struct sched_param *);
static sched_setscheduler_nocheck_type sched_setscheduler_nocheck_ax;
...
/* in nsecs */
#define DEFAULT_INPUT_BOOST_DURATION 5000000
 ...
 */
static unsigned int boost_freq = 1196000;
	...
	case CPUFREQ_GOV_START:
		...
		if (dbs_enable == 1) {
			if (!boost_task_alive) {
				...
				sched_setscheduler_nocheck_ax(boost_task, SCHED_RR, &param);
...
static int __init cpufreq_smartmax_init(void) {
	...
	ignore_nice = DEFAULT_IGNORE_NICE;
        ...
        sched_setscheduler_nocheck_ax = (void*) kallsyms_lookup_name_ax("sched_setscheduler_nocheck");
 
        if(!nr_running_ax || !sched_setscheduler_nocheck_ax)
                return -1;
